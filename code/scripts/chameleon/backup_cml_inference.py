...
def detect(image, sess, show_frames):
    predictions = []
    runtime = 0
...
def detect_cropping(image, sess, target_size, model_default_size, show_frames):
    boxes_global = None
    scores_global = None
    classes_global = None
    num_global = 0
    ...
    for i in range(num_crops_w+1):
        for j in range(num_crops_h+1):
            ...
            start_time = time.time()
            boxes, scores, classes, num, runtime = detect(image_cropped, sess, show_frames)
...
with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
        ...
        for i in range(len(TEST_IMAGE_PATHS)):
            ...
            boxes, scores, classes, num, runtime = detect_cropping(\
				image, sess, target_size, model_default_size, show_frames)
