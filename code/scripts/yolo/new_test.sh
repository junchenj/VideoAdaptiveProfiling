...
for ((index=$begin;index<$begin+$step*$range;index+=$step)); do
	...
	count=$(($count+1))
        ...
        for b in 0 -0.2 -0.1 0.1 0.2; do
				...
				outputfile=$predictions_logs_folder/logs_${video}_bright_${b}_size_${s}_batch_${i}
				if [ -f $outputfile ]; then
					echo Skipping: found $outputfile
					continue
				fi
                                cd $darknet_dir
                                ...
                                sudo mv out_bellevue_${exp_name}.txt $outputfile
                        done
                done
        done
done
