...
#endif
            char buff[256];
	    // JUNCHEN: avoid saving too many weights
...
void test_detector_batch(char *datacfg, char *cfgfile, char *weightfile, char *path, float thresh, float hier_thresh, char *outfile, char *outfolder, int fullscreen, int batch_limit)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
    ...
    if (d)
    {
        ...
        while ((dir = readdir(d)) != NULL)
        {
            ...
            double time1a=what_time_is_it_now();
            image im = load_image_color(input,0,0);
            //printf("%s: Time1a %f seconds.\n", input, what_time_is_it_now()-time1a);
            double time1b=what_time_is_it_now();
            image sized = letterbox_image(im, net->w, net->h);
            //printf("%s: Time1b %f seconds.\n", input, what_time_is_it_now()-time1b);
            ...
            float *X = sized.data;
            //printf("%s: Before %f seconds.\n", input, what_time_is_it_now()-time);
            time=what_time_is_it_now();
            network_predict(net, X);
            printf("%s: Predicted in %f seconds.\n", input, what_time_is_it_now()-time);
            double time2=what_time_is_it_now();
            get_region_boxes(l, im.w, im.h, net->w, net->h, thresh, probs, boxes, masks, 0, 0, hier_thresh, 1);
            //printf("%s: Time2 %f seconds.\n", input, what_time_is_it_now()-time2);
            //if (nms) do_nms_obj(boxes, probs, l.w*l.h*l.n, l.classes, nms);
            if (nms) do_nms_sort(boxes, probs, l.w*l.h*l.n, l.classes, nms);
            double time3=what_time_is_it_now();
            draw_detections(im, l.w*l.h*l.n, thresh, boxes, probs, masks, names, alphabet, l.classes);
            //printf("%s: Time3 %f seconds.\n", input, what_time_is_it_now()-time3);
            if (count % 300 != 0)
                continue; //JUNCHEN: PRINT OUT BBOX FOR 1/90 FRAMES
            if(outfile){
                ...
                char output[80];
                if (outfolder) {
                    strcpy(output, outfolder);
                    strcat(output, "/predictions-");
                } else {
                    strcpy(output, "tmp_predictions/predictions-");
                }
...
void run_detector(int argc, char **argv)
{
    ...
    char *outfolder = find_char_arg(argc, argv, "-outfolder", 0); //JUNCHEN: CHANGE OUTPUT FOLDER
    int batch_limit = find_int_arg(argc, argv, "-limit", -1);
    int *gpus = 0;
    int gpu = 0;
    int ngpus = 0;
    if(gpu_list){
    ...
    else if(0==strcmp(argv[2], "test-batch")) test_detector_batch(datacfg, cfg, weights, filename, thresh, hier_thresh, outfile, outfolder, fullscreen, batch_limit);
